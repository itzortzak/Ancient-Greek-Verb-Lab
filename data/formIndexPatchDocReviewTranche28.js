(function(){
  ...
  function variants(form){ const out=[form]; if(form.includes('(ν)')){ out.push(form.replace('(ν)','')); out.push(form.replace('(ν)','ν')); } if(form.includes('(') && form.includes(')') && !form.includes('(ν)')){ out.push(form.replace(/[()]/g,'')); } return [...new Set(out)]; }
  window.FORM_INDEX = window.FORM_INDEX || {};
  const lemmas = ["εὑρίσκω", "λείπω", "πυνθάνομαι"];
