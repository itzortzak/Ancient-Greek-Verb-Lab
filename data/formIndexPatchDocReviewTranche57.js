(function(){
  ...
  function variants(form){
    const out=[form];
    if(form.includes('(ν)')){ out.push(form.replace('(ν)','')); out.push(form.replace('(ν)','ν')); }
    return [...new Set(out)];
  }
  window.FORM_INDEX = window.FORM_INDEX || {};
  const lemmas = ["αἰτιῶμαι"];
  ...
  const extras = [
    {lemma:"αἰτιῶμαι", form:"ᾐτιάσθην", kind:"finite", tense:"aorist", voice:"passive", mood:"indicative", person:"1sg", number:"sg"}
  ];
  for(const lemma of lemmas){
    const arr = (source[lemma] || []).concat(extras.filter(e => e.lemma===lemma));
