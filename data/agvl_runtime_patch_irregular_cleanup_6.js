(function(){
  const TARGETS = ['ῥίπτω','κεράννυμι','λαμβάνω','λανθάνω','μανθάνω','πάσχω','πίπτω','ἀποθνῄσκω'];
  ...
  function resolveRhipto(slot, entries){
    const [kind, tense, voice, mood, person] = slot.split('|');
    if(kind === 'finite' && tense === 'aorist' && voice === 'passive' && mood === 'indicative' && ['1sg','2sg','3sg','1pl','2pl','3pl'].includes(person)){
      const map = {
        '1sg': [['ἐρρίφθην'], ['ἐρρίφην']],
        '2sg': [['ἐρρίφθης'], ['ἐρρίφης']],
        '3sg': [['ἐρρίφθη'], ['ἐρρίφη']],
        '1pl': [['ἐρρίφθημεν'], ['ἐρρίφημεν']],
        '2pl': [['ἐρρίφθητε'], ['ἐρρίφητε']],
        '3pl': [['ἐρρίφθησαν'], ['ἐρρίφησαν']]
      };
  ...
  function resolveKerannymi(slot, entries){
    const [kind, tense, voice, mood, person] = slot.split('|');
    if(kind === 'finite' && tense === 'imperfect' && voice === 'active' && mood === 'indicative' && person === '1sg'){
      return combine(entries, selectForms(entries, [['ἐκεράννυν'], ['ἐκεράννυον']]));
    }
    if(kind === 'finite' && tense === 'future' && voice === 'active' && mood === 'indicative' && person === '1sg'){
      return combine(entries, selectForms(entries, [['κεράσω'], ['κερῶ']]));
    }
    if(kind === 'finite' && tense === 'aorist' && voice === 'passive' && mood === 'indicative' && person === '1sg'){
      return combine(entries, selectForms(entries, [['ἐκράθην'], ['ἐκεράσθην']]));
    }
    if(kind === 'finite' && tense === 'perfect' && voice === 'middle/passive' && mood === 'indicative' && person === '1sg'){
      return combine(entries, selectForms(entries, [['κέκραμαι'], ['κεκέρασμαι']]));
    }
    return one(entries, entries[0].form);
  }

  function resolveImperative3pl(slot, entries){
    const [kind, tense, voice, mood, person] = slot.split('|');
    if(kind === 'finite' && voice === 'active' && mood === 'imperative' && person === '3pl'){
      return combine(entries, selectForms(entries, [['όντων'], ['έτωσαν']]));
    }
    return one(entries, entries[0].form);
  }
  ...
  const resolvers = {
    'ῥίπτω': resolveRhipto,
    'κεράννυμι': resolveKerannymi,
    'λαμβάνω': resolveImperative3pl,
    'λανθάνω': resolveImperative3pl,
    'μανθάνω': resolveImperative3pl,
    'πάσχω': resolveImperative3pl,
    'πίπτω': resolveImperative3pl,
    'ἀποθνῄσκω': resolveImperative3pl
  };
  ...
  window.AGVL_RUNTIME_PATCH = Object.assign({}, window.AGVL_RUNTIME_PATCH, {
    irregularCleanup6: true,
    irregularCleanup6Lemmas: TARGETS.length,
    irregularCleanup6Stats: stats
  });
})();
