(function(){
  ...
  function entrySig(e){
    return [e.lemma,e.form,e.kind,e.tense,e.voice,e.mood,e.person||'',e.gender||'',e.case||'',e.number||''].join('|');
  }
  function variants(form){
    const out=[form];
    if(form.includes('(ν)')){
      out.push(form.replace('(ν)',''));
      out.push(form.replace('(ν)','ν'));
    }
    return [...new Set(out)];
  }
  window.FORM_INDEX = window.FORM_INDEX || {};
  const lemmas = ['προσεύχομαι','ὑπόκειμαι','πάρειμι'];
