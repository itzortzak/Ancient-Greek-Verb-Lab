(function () {
  ...
  const PERSON_ROWS = [ ['sg','1sg','α΄ ενικό'], ['sg','2sg','β΄ ενικό'], ['sg','3sg','γ΄ ενικό'], ['du','2du','β΄ δυϊκό'], ['du','3du','γ΄ δυϊκό'], ['pl','1pl','α΄ πληθυντικό'], ['pl','2pl','β΄ πληθυντικό'], ['pl','3pl','γ΄ πληθυντικό'] ];
  ...
  const NUMBER_FULL_LABELS = { sg: 'ενικός', du: 'δυϊκός', pl: 'πληθυντικός' };
  const PERSON_LABELS = { '1sg':'α΄ ενικό', '2sg':'β΄ ενικό', '3sg':'γ΄ ενικό', '2du':'β΄ δυϊκό', '3du':'γ΄ δυϊκό', '1pl':'α΄ πληθυντικό', '2pl':'β΄ πληθυντικό', '3pl':'γ΄ πληθυντικό' };
  ...
  const kalosFiniteManifest = window.KALOS_FINITE_MANIFEST || {};
  const kalosFiniteLemmas = Array.isArray(kalosFiniteManifest.lemmas) ? kalosFiniteManifest.lemmas : [];
  const kalosFiniteUnavailableLemmas = new Set(['χρή']);
  const hasKalosLemmaSet = new Set(kalosFiniteLemmas.filter(lemma => !kalosFiniteUnavailableLemmas.has(lemma)));
  ...
  function stripQuantityMarks(str) {
    return String(str || '').normalize('NFD').replace(/[\u0304\u0306]/g, '').normalize('NFC');
  }

  function displayForm(str) {
    return stripQuantityMarks(String(str || '').replace(/\s+/g, ' ').trim());
  }

  function uniqueDisplayForms(forms) {
    return [...new Set((forms || []).map(displayForm).filter(Boolean))];
  }
  ...
  function getParadigmSource(lemma) {
    ...
    if (hasKalosLemmaSet.has(lemma)) return 'kalos';
    return 'missing';
  }
  ...
  function getParadigmEntries(lemma) {
    const entries = []
      .concat(Array.isArray(manualParadigms[lemma]) ? manualParadigms[lemma] : [])
      .concat(Array.isArray(generatedParadigms[lemma]) ? generatedParadigms[lemma] : []);
    const seen = new Set();
    const merged = [];
    for (const entry of entries) {
    ...
    return filterContractedEntries(lemma, merged);
  }
  function getDisplayLemma(lemma) {
    const meta = verbMeta[lemma] || {};
    if (meta.displayLemma) return stripQuantityMarks(meta.displayLemma);
    const contracted = deriveContractedLemma(lemma);
    return stripQuantityMarks(contracted || lemma);
  }
  ...
  const visibleKalosFiniteLemmas = kalosFiniteLemmas.filter(lemma => !kalosFiniteUnavailableLemmas.has(lemma));
  const lemmaList = [...new Set([...Object.keys(manualParadigms), ...Object.keys(generatedParadigms), ...Object.keys(verbMeta), ...visibleKalosFiniteLemmas])].sort((a,b)=>(getDisplayLemma(a)).localeCompare(getDisplayLemma(b),'el'));
  ...
  function collectCellForms(forms, number, person) {
    ...
    const hits = forms.filter(f => f.number === number && normalizePerson(f.number, f.person) === targetPerson).map(f => displayForm(f.form));
    return [...new Set(hits)];
  }
  ...
  function renderInfinitives(nonFinite) {
    ...
    const grouped = {};
    for (const f of infinitives) { const key = `${f.tense}|${f.voice}`; grouped[key] = grouped[key] || []; grouped[key].push(displayForm(f.form)); }
    ...
    const rows = tenses.map(tense => `<tr><th scope="row">${TENSE_LABELS[tense] || tense}</th>${voices.map(voice=>{
      const key=`${tense}|${voice}`; const forms=uniqueDisplayForms(grouped[key]||[]); return `<td class="${forms.length?'':'empty'}">${forms.length?forms.join(' · '):'—'}</td>`;
    }).join('')}</tr>`).join('');
    return `
      ...
      const tables = ['sg','pl'].map(number => {
        const body = CASE_ORDER.map(kase => {
          const cells = GENDER_ORDER.map(gender => {
            const hits = forms.filter(f => f.gender === gender && (f.number || 'sg') === number && (f.case || 'nom') === kase).map(f => displayForm(f.form));
            const uniq = uniqueDisplayForms(hits);
  ...
  let pendingLemmaRenderToken = 0;
  async function renderLemma(lemma) {
    const token = ++pendingLemmaRenderToken;
    updateLemmaPosition(lemma);
    updateLemmaMeta(lemma);
    if (kalosFinite.enabled && hasKalosLemmaSet.has(lemma) && !generatedParadigms[lemma]) {
      finiteSection.innerHTML = '<section class="card"><div class="empty-state">Φόρτωση πεπερασμένων τύπων από το Kalos…</div></section>';
      nonFiniteSection.innerHTML = '<section class="card"><div class="empty-state">Δεν καταγράφονται ακόμη μη πεπερασμένοι τύποι από την εισαγωγή Kalos.</div></section>';
      await ensureKalosLemmaLoaded(lemma);
      if (token !== pendingLemmaRenderToken) return;
    }
    renderFinite(lemma);
  ...
  const kalosFinite = {
    enabled: kalosFiniteLemmas.length > 0,
    basePath: kalosFiniteManifest.basePath || 'data/kalos_finite',
    formBucketCount: kalosFiniteManifest.formBucketCount || 0,
    lemmaBucketCount: kalosFiniteManifest.lemmaBucketCount || 0,
    loadedFormBuckets: new Set(),
    pendingFormBuckets: new Map(),
    loadedLemmaBuckets: new Set(),
    pendingLemmaBuckets: new Map(),
    everActivated: false
  };
  const KALOS_TENSE_CODES = kalosFiniteManifest.tenseCodes || ['present', 'imperfect', 'future', 'aorist', 'perfect', 'pluperfect'];
  const KALOS_VOICE_CODES = kalosFiniteManifest.voiceCodes || ['active', 'middle', 'passive'];
  const KALOS_MOOD_CODES = kalosFiniteManifest.moodCodes || ['indicative', 'subjunctive', 'optative', 'imperative'];
  const KALOS_PERSON_CODES = kalosFiniteManifest.personCodes || ['1sg', '2sg', '3sg', '1pl', '2pl', '3pl', '2du', '3du'];
  ...
  function analysisDisplaySignature(entry) {
    return [entry.lemma || '', displayForm(entry.form || ''), labelFor(entry)].join('|');
  }
  ...
  function bucketForNormalizedKey(key, bucketCount = kaikkiStage2.bucketCount) {
    if (!key || !bucketCount) return 0;
    let hash = 0;
    const chars = [...String(key)].slice(0, 12);
    for (const ch of chars) hash = ((hash * 33) + ch.codePointAt(0)) % bucketCount;
    return hash;
  }
  ...
  async function ensureKaikkiBucketLoaded(normalizedKey) {
    if (!kaikkiStage2.enabled || !kaikkiStage2.bucketCount || !normalizedKey) return { loaded: false, bucketId: null };
    const bucketId = bucketForNormalizedKey(normalizedKey, kaikkiStage2.bucketCount);
  ...
  function decodeKalosFiniteEntry(lemma, record) {
    if (!lemma || !Array.isArray(record) || record.length < 5) return null;
    const form = record[0];
    const tense = KALOS_TENSE_CODES[record[1]];
    const voice = KALOS_VOICE_CODES[record[2]];
    const mood = KALOS_MOOD_CODES[record[3]];
    const person = KALOS_PERSON_CODES[record[4]];
    if (!form || !tense || !voice || !mood || !person) return null;
    const number = person.endsWith('pl') ? 'pl' : (person.endsWith('du') ? 'du' : 'sg');
    return {
      lemma,
      form,
      kind: 'finite',
  ...
  function mergeKalosFormShard(shard) {
    if (!shard || typeof shard !== 'object') return;
    for (const [key, arr] of Object.entries(shard)) {
      if (!Array.isArray(arr) || !arr.length) continue;
      ...
      const seen = new Set(bucket.map(entrySignature));
      for (const item of arr) {
        const lemma = kalosFiniteLemmas[item[0]];
        const decoded = decodeKalosFiniteEntry(lemma, [item[1], item[2], item[3], item[4], item[5]]);
        if (!decoded) continue;
        const sig = entrySignature(decoded);
        if (seen.has(sig)) continue;
        seen.add(sig);
        bucket.push(decoded);
      }
  ...
  function mergeKalosLemmaShard(shard) {
    if (!shard || typeof shard !== 'object') return;
    for (const [id, arr] of Object.entries(shard)) {
      const lemma = kalosFiniteLemmas[Number(id)];
      if (!lemma || !Array.isArray(arr) || !arr.length) continue;
      const bucket = generatedParadigms[lemma] = Array.isArray(generatedParadigms[lemma]) ? generatedParadigms[lemma] : [];
      const seen = new Set(bucket.map(entrySignature));
      for (const item of arr) {
        const decoded = decodeKalosFiniteEntry(lemma, item);
        if (!decoded) continue;
        const sig = entrySignature(decoded);
        if (seen.has(sig)) continue;
        seen.add(sig);
  ...
  async function ensureKalosFormBucketLoaded(normalizedKey) {
    if (!kalosFinite.enabled || !kalosFinite.formBucketCount || !normalizedKey) return { loaded: false, bucketId: null };
    const bucketId = bucketForNormalizedKey(normalizedKey, kalosFinite.formBucketCount);
    if (kalosFinite.loadedFormBuckets.has(bucketId)) return { loaded: false, bucketId };
    if (kalosFinite.pendingFormBuckets.has(bucketId)) {
      await kalosFinite.pendingFormBuckets.get(bucketId);
      return { loaded: true, bucketId };
    }
    const file = `${kalosFinite.basePath}/form_index_bucket_${String(bucketId).padStart(3, '0')}.json`;
    const promise = fetch(file)
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
      ...
      .then(data => {
        mergeKalosFormShard(data);
        kalosFinite.loadedFormBuckets.add(bucketId);
        kalosFinite.everActivated = true;
      })
      .catch(err => {
        console.warn('Kalos finite form bucket load failed:', bucketId, err);
      })
      .finally(() => {
        kalosFinite.pendingFormBuckets.delete(bucketId);
      });
    kalosFinite.pendingFormBuckets.set(bucketId, promise);
    await promise;
    return { loaded: kalosFinite.loadedFormBuckets.has(bucketId), bucketId };
  }

  async function ensureKalosLemmaLoaded(lemma) {
    if (!kalosFinite.enabled || !kalosFinite.lemmaBucketCount || !lemma || !hasKalosLemmaSet.has(lemma)) return { loaded: false, bucketId: null };
    if (generatedParadigms[lemma]) return { loaded: false, bucketId: bucketForNormalizedKey(normalizeGreek(lemma), kalosFinite.lemmaBucketCount) };
    const normalizedLemma = normalizeGreek(lemma);
    const bucketId = bucketForNormalizedKey(normalizedLemma, kalosFinite.lemmaBucketCount);
    if (kalosFinite.loadedLemmaBuckets.has(bucketId)) return { loaded: false, bucketId };
    if (kalosFinite.pendingLemmaBuckets.has(bucketId)) {
      await kalosFinite.pendingLemmaBuckets.get(bucketId);
      return { loaded: true, bucketId };
    }
    const file = `${kalosFinite.basePath}/lemma_bucket_${String(bucketId).padStart(3, '0')}.json`;
    const promise = fetch(file)
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
      ...
      .then(data => {
        mergeKalosLemmaShard(data);
        kalosFinite.loadedLemmaBuckets.add(bucketId);
        kalosFinite.everActivated = true;
      })
      .catch(err => {
        console.warn('Kalos finite lemma bucket load failed:', bucketId, err);
      })
      .finally(() => {
        kalosFinite.pendingLemmaBuckets.delete(bucketId);
      });
    kalosFinite.pendingLemmaBuckets.set(bucketId, promise);
    await promise;
    return { loaded: kalosFinite.loadedLemmaBuckets.has(bucketId), bucketId };
  }
  ...
  async function analyzeForm(query) {
    ...
    const kalosInfo = await ensureKalosFormBucketLoaded(normalized);
    ...
    const kalosNote = kalosInfo.loaded ? ' · φορτώθηκε επιπλέον shard Kalos finite' : (kalosFinite.everActivated ? ' · διαθέσιμη και επέκταση Kalos finite' : '');
    const fallbackNote = usedFallback ? ' · χρησιμοποιήθηκε εναλλακτική σάρωση παραδειγμάτων' : '';
    analyzeStatus.textContent = `Αναζήτηση για: ${raw}${kalosNote}${stage2Note}${fallbackNote}`;
    if (!hits.length) {
    ...
    const rows = hits.map(h => {
      ...
      return `<tr>
      <td>${escapeHtml(displayForm(h.form || raw))}</td>
  ...
  const gameLemmaPool = lemmaList.filter(lemma => {
    ...
    return new Set(entries.map(entry => displayForm(entry.form))).size >= 4;
  });
  ...
  function similarityScore(correct, candidate) {
    ...
    if (displayForm(candidate.form) === displayForm(correct.form)) score -= 1000;
    return score;
  }
  ...
  function pickPedagogicalDistractors(correct, entries) {
    ...
    for (const entry of entries) {
      if (!entry.form || displayForm(entry.form) === displayForm(correct.form)) continue;
      const displayKey = displayForm(entry.form);
      const prev = uniqueByForm.get(displayKey);
      if (!prev || similarityScore(correct, entry) > similarityScore(correct, prev)) uniqueByForm.set(displayKey, entry);
    }
    ...
    return [...primary, ...secondary].slice(0, 3).map(entry => displayForm(entry.form));
  }
  ...
  function buildQuestion(context = {}) {
    ...
    for (let tries = 0; tries < 80; tries++) {
      ...
      if (new Set(candidateEntries.map(entry => displayForm(entry.form))).size < 4) continue;
      ...
      const fallbackDistractors = shuffled(candidateEntries.filter(entry => displayForm(entry.form) !== displayForm(candidateCorrect.form)))
        .map(entry => displayForm(entry.form))
        .filter((form, index, arr) => arr.indexOf(form) === index && !distractors.includes(form))
        .slice(0, 6);
      const optionSet = [...new Set([displayForm(candidateCorrect.form), ...distractors, ...fallbackDistractors])].slice(0, 4);
    ...
    gameState.current = {
      ...
      correctForm: displayForm(correct.form),
  ...
  lemmaSelect.addEventListener('change', () => { void renderLemma(lemmaSelect.value); });
